...
-- =============================================
-- EMOTIONAL MILESTONES (Memory Timeline)
-- =============================================
...
CREATE POLICY "Users can view emotional milestones in their family"
  ON emotional_milestones FOR SELECT
  USING (family_id IN (SELECT family_id FROM profiles WHERE id = auth.uid()));

CREATE POLICY "Users can manage emotional milestones in their family"
  ON emotional_milestones FOR ALL
  USING (family_id IN (SELECT family_id FROM profiles WHERE id = auth.uid()));
